...
from utils.object_detection.detr.convert_to_coco_detection import (
...
class MyDataLoader(DataLoader):
    def __init__(self, dataset, collate_fn, batch_size, shuffle=False):
        super().__init__(
            dataset, collate_fn=collate_fn, batch_size=batch_size, shuffle=shuffle
        )
...
def visualize_predictions(
    image, outputs, threshold=0.9, keep_highest_scoring_bbox=False
):
...
def outputs_to_bbox(image, outputs, threshold=0.9):
    probas = outputs.logits.softmax(-1)[0, :, :-1]
    keep = probas.max(-1).values > threshold
    bboxes_scaled = rescale_bboxes(outputs.pred_boxes[0, keep].cpu(), image.size)
